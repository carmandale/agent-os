...
if ! rg -n "<roadmap_spec_first>" "$ROOT_DIR/instructions/core/plan-product.md" "$ROOT_DIR/instructions/core/create-spec.md" >/dev/null 2>&1; then
  echo "❌ plan-product.md/create-spec.md missing roadmap/spec-first policy"; CODE=1
fi
if ! rg -n "<no_default_fallbacks>" "$ROOT_DIR/instructions/core/plan-product.md" "$ROOT_DIR/instructions/core/create-spec.md" >/dev/null 2>&1; then
