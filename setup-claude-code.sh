...
echo "📥 Installing Claude Code hooks..."
    ...
    if [ -f "$HOME/.agent-os/hooks/install-hooks.sh" ] && [ "$OVERWRITE_COMMANDS" = false ]; then
        ...
        # Download bash hooks
        for hook in stop-hook.sh user-prompt-submit-hook.sh pre-bash-hook.sh post-bash-hook.sh post-tool-use-hook.sh task-status-sync.sh notify-hook.sh install-hooks.sh; do
